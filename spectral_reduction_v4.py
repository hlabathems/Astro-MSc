...
#'''========== Import Data =========='''
    ...
    hdulist = fits.open('spectra/spec-' + plate[j] + '-' + mjd[j] + '-' + fiberid[j] + '.fits')
...
#pl.plot(wl, t2(wl), 'r-')
    ...
    pl.xlabel('Wavelength ($\\rm \AA$)', fontsize='x-large')
#    pl.xlim(4840, 5030)
#pl.ylabel('Normalized Flux')
    pl.ylabel('Flux Density ($\\rm \\times 10^{-17} \ erg \ cm^{-2} s^{-1} \AA^{-1}$)', fontsize='x-large')
    pl.title('Deredshift and Reddening Correction\nspec-' + plate[j] + '-' + mjd[j] + '-' + fiberid[j] + '.fits')
    pl.legend(loc='best')
    pl.savefig('figure/fig_'+ str(j) +'_spec-' + plate[j] + '-' + mjd[j] + '-' + fiberid[j])
    #pl.show()
    pl.close('all')
#pl.figure(2)
